...
function Header({ index, title, description }) {
 ...
 export default function Generator(props) {
  ...
  function updateMuscles(muscleGroup) {
    ...
    if (muscles.length > 2) { // Restrict muscle group selection to 3
      return;
    }
  ...
  return (
    <SectionWrapper id={'generate'} header={"Generate your workout"} title={["It's", "Gym", "o'clock"]}>
      <Header 
        index={'01'} 
      ...
      <Header 
        ...
        description={"Select the muscles for exploding"} 
      />
        ...
        {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
          return (
            <button 
              ...
              className={'bg-slate-950 border border-blue-400 duration-200 hover:border-blue-600 py-3 rounded-lg px-4 ' + (scheme === goal ? 'border-blue-600' : 'border-blue-400')} 
              key={schemeIndex}
            >
